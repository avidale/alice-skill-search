...
class Searcher:
    def __init__(self, k=1.5, b=0.75, stemmer=None):
        self.k = k
        self.b = b
        self.stemmer = stemmer or SnowballStemmer(language='russian')
    ...
    def rerank(self, query, tfidfs, n=10, min_mult_from_top=0.9):
        pairs = tfidfs.most_common(n)
        new_rank = []
        query_stems = self.tokenize(query, stem=True)
        query_words = self.tokenize(query, stem=False)

        for doc_id, score in pairs:
            doc = self.paragraphs[doc_id]

            doc_stems = self.tokenize(doc, stem=True)  # todo: why do it over and over again?
            doc_words = self.tokenize(doc, stem=False)
